...
Guidance for AI agents (and humans) working in this ZMK user-config repository.
Currently ships firmware for the **Geulis** keyboard by karnadii
(<https://github.com/karnadii/geulis/>); additional boards slot in beside it
under `boards/arm/<keyboard>/`.

## What this repository is

This is a **ZMK user-config repo**, not standalone firmware. Each board under
`boards/arm/<keyboard>/` defines its physical layout, matrix transform,
keymap, and ZMK config — and is consumed by the upstream `zmkfirmware/zmk`
build system via `config/west.yml`. There is no application code to compile
locally.
...
1. Create `boards/arm/<keyboard>/` with the standard ZMK board files:
   `Kconfig`, `Kconfig.board`, `Kconfig.defconfig`, `board.cmake`,
   `<keyboard>.dts`, `<keyboard>.yaml`, `<keyboard>_defconfig`. The
   parent directory **must** be a Zephyr ARCH root (e.g. `arm`,
   `nordic`) because Zephyr derives the architecture from the board's
   parent directory; a custom vendor name like `karnadii` fails with
   `Could not find ARCH=... for BOARD=...`.
2. Use a unique `compatible = "karnadi,<keyboard>"` vendor prefix in
   the board DTS so it doesn't collide with upstream shields.
3. Add a `keymap-drawer/<keyboard>.svg` (and `.yaml`) — generated by CI.
4. Append a `board:` + `snippet:` entry to `build.yaml` so CI builds it.
5. Update the path filter in `.github/workflows/draw-keymaps.yml` so
   pushes to the new keymap trigger a re-render.
6. Keep `boards/arm/geulis/` as a reference — it's the most complete
   example (matrix scan, encoders, RGB, OLED, ext-power, ADC, battery).
...
├── config/west.yml           # West manifest pointing at zmkfirmware/zmk @ v0.3
├── zephyr/module.yml         # Registers this repo as a Zephyr module (board_root: .)
├── boards/arm/geulis/        # Geulis board definition (Alice, nRF52840)
...
│   ├── geulis.dts            # Board DTS: kscan, encoders, RGB SPI, OLED I2C, ext-power, ADC
...
├── keymap-drawer/            # Generated visual keymaps (auto-updated by CI)
│   ├── config.yaml           # keymap-drawer rendering config
│   ├── geulis.svg            # Rendered Geulis keymap SVG (do not edit by hand)
│   └── geulis.yaml           # Parsed Geulis keymap YAML (do not edit by hand)
...
The Geulis is the first board in this repo. New boards should add their own
`## Keyboard hardware (<name> specifics)` section below this one — copy the
bullet list and adjust to match the new wiring.
...
- **OLED display:** SSD1306 128×32 panel on I2C0 (SDA = P0.15, SCL = P0.17, address `0x3C`). Toggle via `GEULIS_OLED_ON` in `geulis_options.h`. ZMK's built-in status screen widget is enabled by default when the SSD1306 devicetree node is present.
...
- **GPIO LEDs (P1.10 blue, P1.11 green):** declared in the devicetree `leds { ... }` block but not driven by any feature on this branch — reserved for future use. The previous `zmk-indicator-leds` module that toggled these from layer state has been removed; if you want a layer- or Caps-Lock-driven indicator, switch to the main ZMK branch where the upstream driver is built in.
